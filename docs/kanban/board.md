# Kanban Board

WIP limit: 5 active delegates. Update cards with evidence links.

## To Do
- `docs/kanban/cards/retro-2026-01-11c.md`
- `docs/kanban/cards/retro-2026-01-11c-meadows.md`
- `docs/kanban/cards/retro-2026-01-11c-dorigo.md`
- `docs/kanban/cards/retro-2026-01-11c-merge.md`
- `docs/kanban/cards/web-research-codex-pi-mono.md`
- `docs/kanban/cards/bdd-feature-pivot.md`
- `docs/kanban/cards/cli-executable.md`
- `docs/kanban/cards/mobile-apk.md`
- `docs/kanban/cards/pi-mono-integration-contract.md`
- `docs/kanban/cards/llm-tldr-setup.md`
- `docs/kanban/cards/agents-md-skills.md`
- `docs/kanban/cards/create-skill-acquisition.md`
- `docs/kanban/cards/create-skill-llm-tldr.md`
- `docs/kanban/cards/agents-md-cleanup.md`
- `docs/kanban/cards/behave-bdd-setup.md`
- `docs/kanban/cards/folder-structure-linter.md`

## In Progress
- (empty)

## Blocked
- (empty)

## Done
- `docs/kanban/cards/feature-agent-monitoring-processes.md` — Done: added process list/log viewer UI, BDD steps, and acceptance evidence.
- `docs/kanban/cards/feature-agent-monitoring-logs.md` — Done: added task view log stream UI, BDD steps, and acceptance evidence.
- `docs/kanban/cards/feature-codex-cli-auth-prompt.md`
- `docs/kanban/cards/feature-codex-cli-authenticated.md`
- `docs/kanban/cards/feature-codex-cli-missing.md`
- `docs/kanban/cards/feature-github-pr-happy-path.md` — Done: added Create PR modal, base branch selection, PR status/evidence, and BDD coverage.
- `docs/kanban/cards/feature-github-pr-missing-auth.md` — Done: added missing-auth instructions, PR guardrails, and BDD evidence.
- `docs/kanban/cards/feature-github-pr-missing-branch.md` — Done: added missing base branch validation/error messaging and BDD evidence.
- `docs/kanban/cards/feature-azure-pr-happy-path.md` — Done: added Azure DevOps PR flow, CLI readiness, and BDD evidence.
- `docs/kanban/cards/feature-azure-pr-missing-cli.md` — Done: added Azure CLI missing guidance, PR guardrails, and BDD evidence.
- `docs/kanban/cards/feature-azure-pr-unsupported-repo.md` — Done: added unsupported repo messaging, PR guardrails, and BDD evidence.
- `docs/kanban/cards/feature-code-review-diff-view.md` — Done: added diff review modal, file list, and BDD evidence.
- `docs/kanban/cards/feature-code-review-line-comments.md` — Done: added line comment UI/logic, review comment list, and BDD coverage.
- `docs/kanban/cards/feature-code-review-submit.md` — Done: added send feedback flow, evidence capture, and BDD coverage.
- `docs/kanban/cards/feature-code-review-no-diffs.md` — Done: added no-diff review task, empty-state message, and BDD evidence.
- `docs/kanban/cards/feature-copilot-cli-enable.md`
- `docs/kanban/cards/feature-copilot-cli-autocomplete.md`
- `docs/kanban/cards/feature-copilot-cli-not-installed.md` — Done: added install instructions messaging, enablement guard, and BDD evidence.
- `docs/kanban/cards/feature-copilot-cli-not-authenticated.md` — Done: added auth-required messaging coverage, enablement guard, and BDD evidence.
- `docs/kanban/cards/feature-task-orchestration-create-task.md`
- `docs/kanban/cards/feature-task-orchestration-create-start.md`
- `docs/kanban/cards/feature-task-orchestration-title-required.md`
- `docs/kanban/cards/feature-task-orchestration-cancel.md`
- `docs/kanban/cards/feature-task-orchestration-autocomplete.md`
- `docs/kanban/cards/feature-kanban-board-columns.md`
- `docs/kanban/cards/feature-kanban-board-status-updates.md`
- `docs/kanban/cards/feature-kanban-board-manual-move.md`
- `docs/kanban/cards/feature-kanban-board-search.md`
- `docs/kanban/cards/feature-device-pairing-qr-happy-path.md`
- `docs/kanban/cards/feature-device-pairing-manual-link.md`
- `docs/kanban/cards/feature-device-pairing-reject.md`
- `docs/kanban/cards/feature-device-pairing-invalid-link.md`
- `docs/kanban/cards/feature-device-pairing-repair.md`
- `docs/kanban/cards/demo-assets-gap.md`
- `docs/kanban/cards/demo-assets-production.md`
- `docs/kanban/cards/sprint-review-2026-01-11.md`
- `docs/kanban/cards/qr-connect-mobile-review.md`
- `docs/kanban/cards/acceptance-test-automation.md`
- `docs/kanban/cards/audit-acceptance-evidence.md`
- `docs/kanban/cards/audit-file-length.md`
- `docs/kanban/cards/blind-user-test-0001.md`
- `docs/kanban/cards/ci-pipeline-minimal.md`
- `docs/kanban/cards/ci-sanity-check.md`
- `docs/kanban/cards/docs-index-refresh.md`
- `docs/kanban/cards/exploratory-test-session-0001.md`
- `docs/kanban/cards/kanban-maintenance.md`
- `docs/kanban/cards/kanban-refresh-2026-01-11.md`
- `docs/kanban/cards/kanban-refresh-final.md`
- `docs/kanban/cards/onboarding-setup-docs.md`
- `docs/kanban/cards/ops-runbook-monitoring.md`
- `docs/kanban/cards/quality-attributes-evidence.md`
- `docs/kanban/cards/retro-2026-01-11-dorigo.md`
- `docs/kanban/cards/retro-2026-01-11-meadows.md`
- `docs/kanban/cards/retro-2026-01-11-merge.md`
- `docs/kanban/cards/retro-2026-01-11.md`
- `docs/kanban/cards/retro-2026-01-11b.md`
- `docs/kanban/cards/retro-2026-01-11b-meadows.md`
- `docs/kanban/cards/retro-2026-01-11b-dorigo.md`
- `docs/kanban/cards/retro-2026-01-11b-merge.md`
- `docs/kanban/cards/security-threat-model.md`
- `docs/kanban/cards/slice-0001-acceptance-run.md`
- `docs/kanban/cards/slice-0001-architecture-spike.md`
- `docs/kanban/cards/slice-0001-data-model-align.md`
- `docs/kanban/cards/slice-0001-definition.md`
- `docs/kanban/cards/slice-0001-frontend-ui.md`
- `docs/kanban/cards/slice-0001-thin-poc.md`
- `docs/kanban/cards/slice-definition-checklist.md`
- `docs/kanban/cards/split-delegate-docs.md`
- `docs/kanban/cards/split-delegate-script.md`
- `docs/kanban/cards/test-strategy-bootstrap.md`
- `docs/kanban/cards/user-basics-docs.md`
- `docs/kanban/cards/release-checklist-update.md`
- `docs/kanban/cards/update-setup-docs-tests.md`
- `docs/kanban/cards/release-scope-definition.md`
- `docs/kanban/cards/product-validation.md`
- `docs/kanban/cards/metrics-tracking.md`
- `docs/kanban/cards/ci-status-waiver.md`
- `docs/kanban/cards/indexes-update.md`
- `docs/kanban/cards/metrics-baseline-run.md`
- `docs/kanban/cards/release-checklist-finalize.md`
- `docs/kanban/cards/kanban-refresh-release.md`
