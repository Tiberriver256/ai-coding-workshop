...
const assistantProviderKey = 'slice0001.assistant_provider'; const defaultAttemptStatus = 'running'; const githubRemoteBranches = ['main', 'develop', 'release'];
...
function openPullRequestModal(taskId) { if (!prModal) { return; } const task = findTaskById(taskId); if (!task) { return; } const ready = isGithubCliReady(githubSettings); if (!ready) { activePullRequestTaskId = null; setGithubCliInstructions(githubSettings); setPullRequestFormEnabled(false); prModal.setAttribute('aria-hidden', 'false'); prModal.setAttribute('data-state', 'open'); return; } setGithubCliInstructions(githubSettings); setPullRequestFormEnabled(true); activePullRequestTaskId = task.id; if (prTitleInput) { prTitleInput.value = task.title || ''; } if (prDescriptionInput) { prDescriptionInput.value = task.description || ''; } if (prBaseBranchSelect) { prBaseBranchSelect.value = task.pullRequest?.baseBranch || prBaseBranchSelect.value || 'main'; } setBaseBranchError(''); prModal.setAttribute('aria-hidden', 'false'); prModal.setAttribute('data-state', 'open'); prTitleInput?.focus(); }
function closePullRequestModal() { if (!prModal) { return; } prModal.setAttribute('aria-hidden', 'true'); prModal.setAttribute('data-state', 'closed'); if (prForm) { prForm.reset(); } setGithubCliInstructions(); setPullRequestFormEnabled(true); setBaseBranchError(''); activePullRequestTaskId = null; }
