...
function openPullRequestModal(taskId) { if (!prModal) { return; } const task = findTaskById(taskId); if (!task) { return; } const ready = isGithubCliReady(githubSettings); if (!ready) { activePullRequestTaskId = null; setGithubCliInstructions(githubSettings); setPullRequestFormEnabled(false); prModal.setAttribute('aria-hidden', 'false'); prModal.setAttribute('data-state', 'open'); return; } setGithubCliInstructions(githubSettings); setPullRequestFormEnabled(true); activePullRequestTaskId = task.id; if (prTitleInput) { prTitleInput.value = task.title || ''; } if (prDescriptionInput) { prDescriptionInput.value = task.description || ''; } if (prBaseBranchSelect) { prBaseBranchSelect.value = task.pullRequest?.baseBranch || prBaseBranchSelect.value || 'main'; } prModal.setAttribute('aria-hidden', 'false'); prModal.setAttribute('data-state', 'open'); prTitleInput?.focus(); }
function closePullRequestModal() { if (!prModal) { return; } prModal.setAttribute('aria-hidden', 'true'); prModal.setAttribute('data-state', 'closed'); if (prForm) { prForm.reset(); } setGithubCliInstructions(); setPullRequestFormEnabled(true); activePullRequestTaskId = null; }
...
if (prForm) { prForm.addEventListener('submit', (event) => { event.preventDefault(); const task = findTaskById(activePullRequestTaskId); if (!task) { return; } if (!isGithubCliReady(githubSettings)) { setGithubCliInstructions(githubSettings); setPullRequestFormEnabled(false); return; } const payload = { title: prTitleInput?.value.trim(), description: prDescriptionInput?.value.trim(), baseBranch: prBaseBranchSelect?.value }; createPullRequest(task, payload); closePullRequestModal(); }); }
